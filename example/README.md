...
`memory.test.ts`, `recall.test.ts`, and `governance.test.ts` cover the client methods in isolation. `http.test.ts` covers the HTTP seam, including the cases where the body tries to name another tenant. `mastra.test.ts`, `langchain.test.ts`, and `llamaindex.test.ts` cover the three example-only governed store adapters (`mastraAdapter.ts`, `langchainAdapter.ts`, `llamaindexAdapter.ts`), each routing a framework-shaped vector store through the same governed client so recall stays tenant-bound. `vercel.test.ts` and `openaiAgents.test.ts` cover the two governed-tool adapters (`vercelAdapter.ts`, `openaiAgentsAdapter.ts`), a save and search tool pair for the Vercel AI SDK and the OpenAI Agents SDK, with the tenant closed over at construction rather than passed as a tool argument.
