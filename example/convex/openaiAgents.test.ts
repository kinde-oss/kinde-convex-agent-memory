...
test('an empty-string key mints a fresh uuid (no collision), and a non-empty key is respected', async () => {
  ...
  const first = await t.action(internal.example.openaiSaveMemory, {
    subject: ALICE,
    orgCode: ORG_A,
    key: '',
  ...
  const second = await t.action(internal.example.openaiSaveMemory, {
    subject: ALICE,
    orgCode: ORG_A,
    key: '',
  ...
  const explicit = await t.action(internal.example.openaiSaveMemory, {
    subject: ALICE,
    orgCode: ORG_A,
    key: 'explicit-key',
    content: 'explicit',
